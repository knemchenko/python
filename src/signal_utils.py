...
def generate_signals(
    ...
    Args:
        ...
        forecasts (Dict): The forecasts from the predictor, including 'r_hat'.
                          e.g., {'Arima': {'forecast': pd.Series,
                                           'r_hat': pd.Series}}
        sigma_dict (Dict): A dictionary mapping horizon (int) to its historical
                           RMSE sigma.
    ...
    for h in horizons_to_check:
        ...
        sigma = sigma_dict[h]
        # The signal is confident if the predicted return is > a threshold of its
        # own error
        ...
        if vix_data is not None and not vix_data.empty:
            ...
            if current_vix > vix_percentile_threshold:
                # High VIX, market is fearful, filter out signal
                vix_filter_passed = False

        # --- 4. Volume Filter ---
        volume_filter_passed = True  # Placeholder for now
        # Example logic:
        # if volume_data.iloc[-1] < volume_data.mean():
