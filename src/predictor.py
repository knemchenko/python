...
class Predictor:
    ...
    def __init__(self, ticker: str) -> None:
        ...
        if not os.path.exists(self.model_filepath):
            raise FileNotFoundError(
                f"Model file not found at {self.model_filepath}. "
                "Please run the initial training first."
            )

        self.models: List[BaseModel] = joblib.load(self.model_filepath)
        print(
            f"Successfully loaded {len(self.models)} models for ticker "
            f"{self.ticker} from {self.model_filepath}"
        )
    ...
    def update_and_predict(
        self, full_data: pd.Series, n_periods: int = 30
        ...
        Returns:
            Dict[str, Dict[str, Any]]: A dictionary where keys are model names and
        ...
        print(
            f"\n--- Generating forecasts for {self.ticker} for the next "
            f"{n_periods} days ---"
        )
        ...
        for model in self.models:
            ...
            try:
                ...
                results[model_name] = {"forecast": forecast, "r_hat": r_hat}
