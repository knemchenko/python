...
def main():
    ...
    This is a one-time script to run to build up historical data for metrics calculation.
    """
    ...
    for ticker in config.TICKERS:
        ...
        try:
            ...
            if len(full_data_df) < 200: # Ensure we have enough data to backfill
                print(f"Not enough data for {ticker} to backfill (need > 200 days). Skipping.")
                continue
            ...
            # We stop 30 days before the end to ensure we can generate 30-day forecasts.
            start_index = 200
            ...
            for i in tqdm(range(start_index, end_index)):
                ...
                # Note: This uses the pre-trained models and re-fits them on the slice.
                # This simulates the daily update process.
                forecasts_data = predictor.update_and_predict(full_data=historical_slice_df['Close'], n_periods=30)
                ...
                signals = generate_signals(
                    ...
                    volume_data=historical_slice_df['ma30_volume']
                )

                if signals:
                    # We need to manually set the timestamp to the "current day" of the backfill
    ...
    if all_historical_signals:
        ...
        print(f"\nSuccessfully back-filled and saved {len(all_historical_signals)} signals to {history_path}")
    else:
