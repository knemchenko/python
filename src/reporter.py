...
def format_signals_to_table(signals: List[Dict[str, Any]]) -> Tuple[str, bool]:
    """
    Formats signals into a concise Markdown V2 table.
    Returns the formatted table string and a boolean indicating if there were
    any signals.
    """
    ...
    separator = "—" * 23
    table_lines = ["`H   DIR   Δ%    σ   W%`", f"`{separator}`"]
...
class Reporter:
    ...
    def __init__(self) -> None:
        ...
        self.log_path: str = os.path.join(config.DATA_DIR, "publish_log.json")

    def _log_published_signals(self, signals: List[Dict[str, Any]]) -> None:
    ...
    async def send_report(
        self, ticker: str, signals: List[Dict[str, Any]], plot_path: str
    ) -> None:
        """
        ...
        if not has_signals:
            print(
                f"No signals to report for {ticker} after filtering. "
        ...
        try:
            ...
            print(
                f"Successfully sent report for {ticker} to Telegram chat "
                f"{self.chat_id}"
            )
        except Exception as e:
